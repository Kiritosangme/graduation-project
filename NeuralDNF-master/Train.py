...
def runTraining():
    ...
    argParser.add_argument("-communicationProtocol", type=int, default=2, metavar='',
                           help="Which Communication Protocol to use within the neural network (Default 2)") #用哪种通信方式
    argParser.add_argument("-nbTrainingEpochs", type=int, default=3, metavar='',
    ...
    GraphNet = GGNN(nbIterations = nbIter, embeddingDim=embDim,communicationProtocol=commProt) #(message passing it,dimension,通信方式) to GGNN
    GraphNet.train(data,nbEpochs = nbEp, feedback_period=feedPer, logPrints=logPr, resetWeights=resetW)
#print(data)
def get_available_gpus():#pring gpu information
